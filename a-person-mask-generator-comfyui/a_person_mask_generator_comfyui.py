    ...
    def __get_mask(
        ...
        if len(masks) == 0:
            ...
            for i, mask in enumerate(masks):
                ...
                mask_data = mask.numpy_view()
                while mask_data.ndim > 2:
                    mask_data = mask_data.squeeze()
                
                # Create the mask array directly
                mask_array = mask_background_array.copy()
                
                # Set pixels to white where confidence exceeds threshold
                for c in range(image_shape[-1]):
                    mask_array[:, :, c] = np.where(
                        mask_data > confidence,
                        mask_foreground_array[:, :, c],
                        mask_background_array[:, :, c]
                    )
