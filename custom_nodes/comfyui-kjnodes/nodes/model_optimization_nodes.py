...
def get_sage_func(sage_attention, allow_compile=False):
    logging.warning(f"SageAttention is DISABLED due to GPU compatibility issues. Using PyTorch attention instead.")
    
    # Fallback to PyTorch attention regardless of requested mode
    def pytorch_fallback(q, k, v, is_causal=False, attn_mask=None, tensor_layout="NHD", **kwargs):
        """Fallback function using PyTorch's scaled_dot_product_attention"""
        import torch.nn.functional as F
        
        # Handle tensor layout conversion if needed
        if tensor_layout == "NHD":
            # Already in the right format for SDPA
            pass
    ...
    # Always use the pytorch fallback
    sage_func = pytorch_fallback
